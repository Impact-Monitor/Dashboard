...
const FileManager = ({ onFilesSelected }) => {
  ...
  async function getUsers() {
    let response = await fetch('http://127.0.0.1:3005/GetNextCloud', {
        method: 'GET',
        headers: {
  ...
  // console.log("stateData Before");
  // console.log(stateData);
  // getUsers().then(data => setStateData(formatData(data, intl)));
  // console.log("stateData After");
  // console.log(stateData);

  // Fetch data only once when the component mounts
  React.useEffect(() => {
    getUsers().then(data => setStateData(formatData(data, intl)));
  }, [intl]); // intl as a dependency since it's used inside formatData
  ...
  const handleSelectionChange = (event: GridRowClickEvent) => {
    ...
    onFilesSelected(event.dataItem['path'].toString());
