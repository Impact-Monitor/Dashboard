...
import DragPolarPlotProperties from "./DragPolarPlotProperties";


const DIMENSIONS = { marginTop: 60, marginRight: 60, marginBottom: 60, marginLeft: 60, innerPadding: 0 };


export const DragPolarPlot = forwardRef((props, ref) => {
  ...
  const InitialiseData = () => {
    ...
    for (let k = 0; k < cpacsFiles.length; k++)
    {
        ...
        clArray.push(cl.split(';').map(Number));
        let cd = element2.getElementsByTagName('cd')[0].textContent;
        cdArray.push(cd.split(';').map(Number));

        // dragpolar
        let dragPolar = [];
        for (let i = 0; i < altitudeArray[k].length; i++)
        {
            dragPolar.push([Number(altitudeArray[k][i]), Number(machArray[k][i]), Number(clArray[k][i]), Number(cdArray[k][i])]);
        }
  ...
  const UpdateChart = () => {
    ...
    for (let k = 0; k < dragPolars.length; k++)
    {
      ...
      let CL_Values = [...new Set(clArray[k])];

      let data = [];
      if (mode == "LD vs CL") {
        //this.legend.LegendTitle = "Mach";
        for (let i = 0; i < Mach_Values.length; i++) {
          let curve = AddNewLineSeries_AltitudeMach(k, plotAltitude, Mach_Values[i], mode);
          if (curve.length > 0)
          {
        ...
        for (let i = 0; i < CL_Values.length; i++) {
          let curve = AddNewLineSeries_Drag_vs_Mach(k, plotAltitude, CL_Values[i], mode);
          if (curve.length > 0)
          {
            for (let j = 0; j < curve.length; j++)
              data.push(curve[j]);
  ...
  const AddNewLineSeries_AltitudeMach = (cpacsFileIndex, altitude, machNumber, mode) => {
    let dragPolar = dragPolars[cpacsFileIndex];

    let curve = [];
    if (mode == "LD vs CL") {
      for (let i = 0; i < dragPolar.length; i++) {
        if (dragPolar[i][0] == altitude && dragPolar[i][1] == machNumber) {
          curve.push({ designId: "D" + cpacsFileIndex + "Mach = " + machNumber, ser1: dragPolar[i][2], ser2: dragPolar[i][2] / dragPolar[i][3] });
      ...
      for (let i = 0; i < dragPolar.length; i++) {
        if (dragPolar[i][0] == altitude && dragPolar[i][1] == machNumber) {
          curve.push({ designId: "D" + cpacsFileIndex + "Mach = " + machNumber, ser1: dragPolar[i][2], ser2: dragPolar[i][3] });
          // xxx.push(dragPolar[i][2]);
      ...
      for (let i = 0; i < dragPolar.length; i++) {
        if (dragPolar[i][0] == altitude && dragPolar[i][1] == machNumber) {
          curve.push({ designId: "D" + cpacsFileIndex + "Mach = " + machNumber, ser1: dragPolar[i][3], ser2: dragPolar[i][2] });
  ...
  const AddNewLineSeries_Drag_vs_Mach = (cpacsFileIndex, altitude, CL, mode) => {
    let dragPolar = dragPolars[cpacsFileIndex];
    ...
    for (let k = 0; k < xxx.length; k++) {
      curve.push({ designId: "" + 0, ser1: xxx[k], ser2: yyy[k] });
    }
